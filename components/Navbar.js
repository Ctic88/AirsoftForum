...
import { Shield, User, Users, LogOut, Menu, X, Settings, LayoutGrid, Calendar, Info } from 'lucide-react';
        ...
        <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center p-4">
            <div className="glass w-full max-w-6xl h-16 rounded-full flex items-center justify-between px-8 border border-white/10 shadow-2xl">
                <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity shrink-0">
                    ...
                    <span className="font-bold text-xl tracking-tight text-white">AIRSOFT<span className="text-accent-light">FORUM</span></span>
                </Link>
                ...
                <div className="hidden md:flex items-center gap-6">
                    ...
                    {session ? (
                        ...
                        <div className="flex items-center gap-4">
                            <Link href="/login" className="text-sm font-semibold py-2 px-6 rounded-full bg-transparent border border-white/20 hover:bg-white/10 transition-all text-white">Login</Link>
                        ...
                        <div className="bg-[#0b0c0b]/95 backdrop-blur-2xl rounded-[32px] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
                            <div className="p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <Link onClick={() => setIsOpen(false)} href="/tools" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white">
                                        <Settings size={18} className="text-accent-light" /> Tools
                                    </Link>
                                    <Link onClick={() => setIsOpen(false)} href="/wiki" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-center text-white">
                                        <Info size={18} className="text-accent-light" /> Wiki
                                    </Link>
                                    <Link onClick={() => setIsOpen(false)} href="/tips" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white">
                                ...
                                {session && (
                                    <div className="grid grid-cols-2 gap-3">
                                        <Link onClick={() => setIsOpen(false)} href="/forum" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white">
                                            <Calendar size={18} className="text-accent-light" /> Forum
                                        </Link>
                                        <Link onClick={() => setIsOpen(false)} href="/operators" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white">
                                            <Users size={18} className="text-accent-light" /> Operators
                                        </Link>
                                        <Link onClick={() => setIsOpen(false)} href="/loadouts" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white">
                                            <Shield size={18} className="text-accent-light" /> Gear
                                        </Link>
                                        <Link onClick={() => setIsOpen(false)} href="/intel" className="flex items-center gap-3 p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all font-bold text-sm text-white text-center">
                                            <Calendar size={18} className="text-accent-light" /> Intel
                                        </Link>
                                    </div>
                                )}
                                ...
                                <div className="mt-4 pt-4 border-t border-white/5 flex flex-col gap-3">
                                    {session ? (
                                        <>
                                            ...
                                            <div className="grid grid-cols-1 gap-3 mt-2">
                                                <Link
                                                    onClick={() => setIsOpen(false)}
                                                    href="/settings"
                                                    className="flex items-center justify-center gap-2 py-4 glass border border-white/10 rounded-2xl font-bold uppercase tracking-widest text-[10px] hover:bg-white/10 transition-all text-white"
                                                >
                                                    <User size={14} className="text-accent-light" /> Profile Settings
                                                </Link>
                                            </div>
                                            <button
                                                type="button"
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <Link onClick={() => setIsOpen(false)} href="/login" className="py-4 glass border border-white/10 rounded-2xl font-bold uppercase tracking-widest text-center text-xs text-white">Login</Link>
